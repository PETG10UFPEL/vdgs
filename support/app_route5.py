...
st.markdown(
    "<h2 style='text-align:center;'>🗺️ Rota de Visitas</h2>",
    unsafe_allow_html=True,
)
...
for i in range(1, st.session_state.num_stops + 1):
    ...
    # Campo de texto nativo do Streamlit
    st.text_input(
        f"Endereço {i}",
        key=f"addr_{i}",
...
if calc:
    ...
    with st.spinner("Localizando endereços…"):
        ...
        for i, addr in raw:
            ...
            if loc is None:
                st.error(f"❌ Não encontrei a Parada {i}: '{addr}'. Tente simplificar o endereço.")
                st.stop()
            lat, lon = loc
